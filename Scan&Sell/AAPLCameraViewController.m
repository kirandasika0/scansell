...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if ([self.tempSegueDictionary objectForKey:@"purpose"] != nil) {
        self.purposeLabel.text = self.tempSegueDictionary[@"purpose"];
    }
    
    if ([[self.tempSegueDictionary objectForKey:@"purpose_s"] isEqualToString:@"front"]) {
        self.uniqueKey = self.tempSegueDictionary[@"front_cover_image_key"];
    }
    
    if ([[self.tempSegueDictionary objectForKey:@"purpose_s"] isEqualToString:@"first"]) {
        self.uniqueKey = self.tempSegueDictionary[@"first_cover_image_key"];
    }
    if ([[self.tempSegueDictionary objectForKey:@"purpose_s"] isEqualToString:@"back"]) {
        self.uniqueKey = self.tempSegueDictionary[@"back_cover_image_key"];
    }
}
	...
	dispatch_async( self.sessionQueue, ^{
		...
		[self.stillImageOutput captureStillImageAsynchronouslyFromConnection:connection completionHandler:^( CMSampleBufferRef imageDataSampleBuffer, NSError *error ) {
			if ( imageDataSampleBuffer ) {
                ...
                //Getting the UIImage instance of the data
                UIImage *clickedImage = [UIImage imageWithData:imageData];
                //caching the image
                [[SAMCache sharedCache] setImage:clickedImage forKey:self.uniqueKey];
                //closing the view controller to head back to the main view
...
- (IBAction)closeTakePhoto:(id)sender {
    //Closing the take photo view controller
    //to go back to the main view controller
    [self dismissViewControllerAnimated:YES completion:nil];
}
